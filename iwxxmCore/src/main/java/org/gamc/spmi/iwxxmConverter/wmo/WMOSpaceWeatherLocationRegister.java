 ...
 * @author moryakov
 */
public class WMOSpaceWeatherLocationRegister implements WMORegister {

	private static final String registerFileName = "codes.wmo.int-49-2-SpaceWxLocation.rdf";

	TreeMap<String, String> wmoSWXLocations = new TreeMap<String, String>();

	@Override
	public TreeMap<String, String> getContent() {
		// TODO Auto-generated method stub
		return wmoSWXLocations;
	}

	@Override
	public String getWMOUrlByCode(Object code) {
		if (wmoSWXLocations.size() == 0)
			parseWMOXml();

		return wmoSWXLocations.get(code);
	}
	...
	public void parseWMOXml() {
		try (InputStream is = new FileInputStream(registerFileName);) {
			...
			registerLogger.debug("Total members in SpaceWeather locations: " + totalCloudElements);
			...
			for (int i = 0; i < listOfCloudElements.getLength(); i++) {
				...
				if (currentNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					if (notationList != null && notationList.getLength() > 0) {
						Element notation = (Element) notationList.item(0);
						Node entityNode = notation.getElementsByTagName("reg:entity").item(0);
						
						Element entityElement = (Element) entityNode;
						
						url = entityElement.getAttribute("rdf:resource");
						
//						url = notation.getElementsByTagName("reg:entity").item(0).getAttributes()
//								.getNamedItem("rdf:resource").getTextContent();

					}

					if (code.length() > 0 && url.length()>0)
						wmoSWXLocations.put(code, url);
