 ...
 * @author moryakov
 */
public class WMONilReasonRegister implements WMORegister {

	private static final String registerFileName = "codes.wmo.int-common-nil.rdf";

	TreeMap<String, String> wmoReasonCodes = new TreeMap<String, String>();
	...
	public String getWMOUrlByCode(Object code) {
		if (wmoReasonCodes.size() == 0)
			parseWMOXml();

		return wmoReasonCodes.get(code);
	}
	...
	public void parseWMOXml() {
		try (InputStream is = new FileInputStream(registerFileName)) {
			...
			XPathExpression expr = xpath.compile("/RDF/RegisterItem"); // "/RDF/RegisterItem/register/Register/member/Concept");
			...
			registerLogger.debug("Total members in Reasons: " + totalCloudElements);
