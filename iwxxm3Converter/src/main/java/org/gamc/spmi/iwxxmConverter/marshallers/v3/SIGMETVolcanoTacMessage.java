...
/**
 * Implemetation of a SIGMET Tac message for VOLCANIC ASH sigmet (
 * Mostly derived from SIGMETTacMessage, but has its own implementation for phenomena description and location sections
 * @author alex
 */
public class SIGMETVolcanoTacMessage extends SIGMETTacMessage {
	...
	protected StringBuffer fillAndRemoveForecastedLocation(StringBuffer tac) {
		Matcher matcherFcst = SigmetParsingRegexp.sigmetVolcanoForecastSection.matcher(tac);
		int lastMatch = 0;
		int firstMatch = 0;
